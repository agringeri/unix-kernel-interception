...
// Save pointers to original system calls
...
// New intercepted open call
asmlinkage long new_sys_open(const char *pathname, int flags, mode_t mode) {
    int userid = current_uid().val;
    // Only print line to syslog if the userid is over 1000 (not a root user)
    if (userid >= 1000) {
